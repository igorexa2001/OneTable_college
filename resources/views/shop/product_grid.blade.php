    ...
    @foreach($products as $product)
        ...
        <div class="product_item {{$product->is_new == '1' ? 'is_new' : ($product->is_sale == '1' ? 'discount': '')}}">
            ...
            <div class="product_image d-flex flex-column align-items-center justify-content-center">
                @if($product->productPicture->first())
            ...
            <div class="product_content">
                @if ($product->is_sale == 1)
                    <div class="product_price">${{$product->price}}<span>${{$product->price*1.75}}</span></div>
                @else
                ...
                <button class="btn btn-primary buy_button_single" id="buy_button" value="{{$product->id}}" style="cursor: pointer">Buy</button>
            </div>
